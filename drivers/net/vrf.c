...
struct vrf_map_elem {
	...
	/* list of VRFs registered to the specific table */
	struct list_head vrf_list_head;
...
struct net_vrf {
	...
	/* VRFs bound to the same table are grouped together */
...
{
	struct list_head *me_head = &me->vrf_list_head;
	struct net_vrf *vrf;
			      ...
			      int ifindex, int users)
{
	...
	me->ifindex = ifindex;
	me->users = users;
	INIT_LIST_HEAD(&me->vrf_list_head);
}
...
link_vrf:
	...
	list_add(&vrf->me_list, &me->vrf_list_head);
